...
import Estructura.Famosos.FamosoNoExisteException;
...
public class EliminarFamoso extends VentanaPadre {
	 ...
	 * Create the dialog.
	 */
	public EliminarFamoso(final SalonFama salonFama) {
		this.salonFama=salonFama;
	
		eliminar.setSize(106, 23);
		eliminar.setLocation(270, 192);
...
//		this.salonFama = Collections.sort(salonFama);
		...
		eliminar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					eliminar(salonFama);
				} catch (FamosoNoExisteException e1) {
	...
	private void eliminar(final SalonFama salonFama)
			throws FamosoNoExisteException {
		int n = JOptionPane.showOptionDialog(contentPanel,
				"¿Esta seguro de que desea eliminarlo?", "Confirmar",
				JOptionPane.YES_NO_CANCEL_OPTION,
				JOptionPane.QUESTION_MESSAGE, null, null, null);
		
		switch (n) {
		case JOptionPane.YES_OPTION:
			salonFama.eliminar(actualizar());
			JOptionPane.showMessageDialog(contentPanel,
					"Famoso eliminado con exito.");
			dispose();
			break;
		}
	}
